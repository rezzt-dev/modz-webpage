...
export default function AdminDashboard() {
  ...
  const render = () => {
    ...
    const header = `
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8 md:mb-12 border-b border-white/10 pb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold uppercase tracking-tight text-white mb-2">${t('dashboard.title')}</h1>
        ...
        <div class="border border-white/10 bg-surface/20">
            <div class="hidden md:grid grid-cols-12 gap-4 p-4 border-b border-white/10 text-[10px] uppercase tracking-widest text-secondary font-bold">
                <div class="col-span-1">${t('dashboard.columns.id')}</div>
                <div class="col-span-5">${t('dashboard.columns.title')}</div>
                <div class="col-span-2">${t('dashboard.columns.type')}</div>
                <div class="col-span-2">${t('dashboard.columns.version')}</div>
                <div class="col-span-2 text-right">${t('dashboard.columns.actions')}</div>
            </div>
  ...
  const renderProjects = () => {
    ...
    if (countLabel) countLabel.textContent = `${t('dashboard.subtitle')} // ${projects.length}`;

    if (projects.length === 0) {
      if (listContainer) listContainer.innerHTML = `<div class="p-8 text-center text-secondary text-xs">${t('dashboard.noData')}</div>`;
      return;
    }
      ...
      listContainer.innerHTML = projects.map(p => `
            <div class="flex flex-col md:grid md:grid-cols-12 gap-4 p-4 border-b border-white/5 items-start md:items-center hover:bg-white/5 transition-colors text-xs font-mono relative group">
                ...
                <div class="absolute top-4 right-4 md:hidden flex gap-3">
                        <a href="#/admin/edit/${p.id}" class="text-white hover:underline decoration-white/30">${t('dashboard.actions.edit')}</a>
                        <button data-id="${p.id}" class="delete-btn text-red-500 hover:text-red-400">${t('dashboard.actions.del')}</button>
                </div>

                <div class="w-full md:col-span-1 text-secondary opacity-50 text-[10px] truncate flex justify-between md:block">
                    <span class="md:hidden uppercase tracking-widest">${t('dashboard.columns.id')}:</span>
                    ${p.id}
                </div>
          ...
          : `<i class="material-icons text-sm text-white/70 mr-1">${p.icon || 'star'}</i>`}
                ...
                <div class="w-full md:col-span-2 text-secondary uppercase text-[10px] flex justify-between md:block border-b border-white/5 md:border-none pb-2 md:pb-0 mb-2 md:mb-0">
                    <span class="md:hidden font-bold">${t('dashboard.columns.type')}:</span>
                    ${p.type}
                </div>
                ...
                <div class="w-full md:col-span-2 text-secondary flex justify-between md:block border-b border-white/5 md:border-none pb-2 md:pb-0 mb-2 md:mb-0">
                    <span class="md:hidden font-bold text-[10px] uppercase tracking-widest">${t('dashboard.columns.version')}:</span>
                    ${p.version}
                </div>
  ...
  const setupListeners = () => {
    ...
    container.addEventListener('click', (e) => {
      if (e.target.classList.contains('delete-btn')) {
        if (confirm(t('dashboard.actions.confirmDelete'))) {
