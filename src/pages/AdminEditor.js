...
export default function AdminEditor({ id }) {
    ...
    if (isEdit && !project) {
        ...
        errorDiv.innerHTML = t('editor.notFound');
        return errorDiv;
    }
    ...
    const render = () => {
        container.innerHTML = `
            ...
            <div>
                <div class="text-[10px] md:text-xs text-secondary font-mono mb-2 uppercase tracking-widest">${t('editor.root')} / ${isEdit ? t('editor.editEntry') : t('editor.newEntry')}</div>
                <h1 class="text-3xl md:text-4xl font-bold text-white">${t('editor.createContent')}</h1>
            </div>
            <div>
                <a href="#/admin" class="text-[10px] md:text-xs text-secondary hover:text-white uppercase tracking-widest">${t('editor.cancel')}</a>
            </div>
        </div>
        ...
        <form id="editor-form" class="space-y-12 md:space-y-16 max-w-4xl mx-auto">
            ...
            <!-- Section 01 -->
            <div class="space-y-8">
                <h3 class="text-xs font-mono text-secondary uppercase tracking-widest border-b border-white/10 pb-2 w-32">${t('editor.s1')}</h3>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                    <div class="space-y-2 group">
                        <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.projectTitle')}</label>
                     ...
                     <div class="space-y-2 group">
                        <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.category')}</label>
                ...
                <div class="space-y-2 group">
                     <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.shortDesc')}</label>
                 ...
                 <div class="space-y-4 group">
                     <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.iconSelector')}</label>
                     ...
                     <div class="flex items-center gap-4 mt-4 p-4 border border-white/10 bg-white/5 rounded">
                        <span class="text-[10px] text-secondary uppercase">${t('editor.selected')}</span>
            ...
            <!-- Section 02 -->
            <div class="space-y-8">
                <h3 class="text-xs font-mono text-secondary uppercase tracking-widest border-b border-white/10 pb-2 w-48">${t('editor.s2')}</h3>
                
                 <div class="space-y-2 group">
                     <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.problemStatement')}</label>
                ...
                <div class="space-y-2 group">
                     <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.proposedSolution')}</label>
            ...
            <!-- Section 03 -->
            <div class="space-y-8">
                <h3 class="text-xs font-mono text-secondary uppercase tracking-widest border-b border-white/10 pb-2 w-40">${t('editor.s3')}</h3>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                     <div class="space-y-2 group">
                        <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.launcherType')}</label>
                    ...
                    <div class="space-y-2 group">
                        <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.version')}</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mt-8">
                    <div class="space-y-2 group">
                         <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.environment')}</label>
                    ...
                    <div class="space-y-2 group">
                         <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.dependency')}</label>
                ...
                <div class="space-y-2 group mt-8">
                     <label class="block text-[10px] uppercase tracking-widest text-secondary group-focus-within:text-white transition-colors">${t('editor.filePath')}</label>
                     ...
                     <p class="text-[10px] text-secondary/50 pt-1">${t('editor.pathHint')}</p>
                </div>
            </div>

            <div class="pt-12 border-t border-white/10 flex flex-col md:flex-row justify-end gap-4 md:gap-6">
                <a href="#/admin" class="w-full md:w-auto px-8 py-3 text-secondary text-center hover:text-white text-xs uppercase tracking-widest transition-colors">${t('editor.cancel')}</a>
                <button type="submit" class="w-full md:w-auto px-8 py-3 bg-white text-black font-bold text-xs uppercase tracking-widest hover:bg-white/90 transition-colors">
                    ${isEdit ? t('editor.update') : t('editor.deploy')}
                </button>
            </div>
        </form>
        `;
