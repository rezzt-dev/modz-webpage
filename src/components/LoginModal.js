    ...
    <div class="relative w-full max-w-md bg-black border border-white/20 p-8 shadow-2xl transform scale-95 transition-transform duration-200">
        ...
        <form id="modal-login-form" class="space-y-6">
            <div class="space-y-2">
                ...
                <input type="text" id="modal-username" class="w-full bg-surface/50 border border-white/10 p-3 text-xs text-white focus:border-white/50 focus:outline-none transition-colors font-mono" placeholder="${t('auth.placeholderUser')}">
            </div>
            
            <div class="space-y-2">
                <label class="block text-[10px] uppercase tracking-widest text-secondary">${t('auth.password')}</label>
                <input type="password" id="modal-password" class="w-full bg-surface/50 border border-white/10 p-3 text-xs text-white focus:border-white/50 focus:outline-none transition-colors font-mono" placeholder="${t('auth.placeholderPass')}">
            </div>
